...
const BookingDoctor = () => {
  ...
  console.log("render");
  //   console.log("profileDoctorId:", profileDoctorId);
  ...
  //   console.log("totalCommentPages:", totalCommentPages);
  //   console.log("editingIndex:", editingIndex);
  //   console.log("selectedCommentId:", selectedCommentId);
  //   console.log("showChatRoom:", showChatRoom);
  ...
  const handleClose = () => {
    ...
    if (disconnected === false) setDisconnected(true);
  };
    ...
    const loadProfileDoctorById = async () => {
      try {
        ...
        let res = await Apis.get(
          endpoints["load-profile-doctor-by-Id"](profileDoctorId)
        );
  ...
  const loadComment = async () => {
    try {
      //   setLoading(true);
      let res = await Apis.get(
        endpoints["load-comments-page"](profileDoctorId)
      );
      setComment(res.data.content);
        ...
        <div className="Booking_Doctor_Content">
                      ...
                      <>
                        <div className="Profile_Doctor_Total_Rating">
                          <Rating
                            ...
                            name="half-rating-read"
                            defaultValue={(
                              doctorDetail.totalRating /
                              doctorDetail.countRating
                            ).toFixed(1)}
                            precision={0.1}
                            readOnly
                          />
          ...
          <div className="Booking_Doctor_Body">
            <div className="Phonenumber">
              ...
              {current_user === null ? (
                <button>
                  <Link to={`/login?next=/doctor/${profileDoctorId}`}>
                    Đăng nhập để đặt khám
                  </Link>
                </button>
              ) : (
                <button>
                  <Link to={url}>ĐẶT KHÁM NGAY</Link>
                </button>
              )}
              {current_user === null ? (
                <button>
                  <Link to={`/login?next=/doctor/${profileDoctorId}`}>
                    Đăng nhập để nhắn tin cho bác sĩ
                  </Link>
                </button>
              ) : (
            ...
            <div className="Profile_Doctor_Comment">
              ...
              <div className="Comment_Sort_Group">
                ...
                <button className="Comment_Sort_Butt" onClick={loadCommentPage}>
                  Sắp xếp
                </button>
              </div>
              <div className="Comment_Area">
                <div className="Comment_List">
                  <ListGroup>
                    {Object.values(comment).map((c, index) => (
                              ...
                              {editingIndex === index ? (
                                ...
                                <>
                                  {current_user === null ? (
                                    ...
                                    <button
                                      className="Edit_Comment_Butt"
                                      type="button"
                                      onClick={() =>
                                        handleEdit(index, c.commentId)
                                      }
                                    >
                                      Sửa bình luận
                                    </button>
