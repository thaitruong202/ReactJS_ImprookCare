  ...
  useEffect(() => {
    ...
    const loadAll = async () => {
      try {
        ...
        let ses = await authApi().get(
          endpoints["load-custom-timeslot"](res.data[0].profileDoctorId)
        );
    ...
    if (note && selectedDate) {
      if (selectedEvent) {
        ...
        console.log(
          ...
          moment(timeE).format("YYYY-MM-DD HH:mm:ss")
        );
          ...
          <div className="Schedule_Right">
            <div className="Schedule_Right_Content">
              ...
              <div className="Schedule_Profile_Option">
                ...
                <select
                  className="value"
                  defaultValue={selectedProfileDoctorId}
                  onChange={(e) => profileDoctorChange(e)}
                  onFocus={(e) => profileDoctorChange(e)}
                >
                  {Object.values(profileDoctorByUserId).map((pd) => (
                    <option key={pd.profileDoctorId} value={pd.profileDoctorId}>
                      {pd.name}
                    </option>
                  ))}
                </select>
                      ...
                      <Modal.Body className="Schedule_Option">
                        <div className="Schedule_Profile_Option">
                          ...
                          >
                            {Object.values(profileDoctorByUserId).map((pd) => (
                              <option
                                key={pd.profileDoctorId}
                                value={pd.profileDoctorId}
                              >
                                {pd.name}
                              </option>
                            ))}
