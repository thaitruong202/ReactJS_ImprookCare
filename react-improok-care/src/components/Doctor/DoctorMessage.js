  ...
  const loadProfileDoctor = async () => {
    try {
      let res = await Apis.get(
        endpoints["load-profile-doctor-by-userId"](current_user?.userId)
      );
        ...
        {profileDoctor.map((pd) => (
          <ListItem disableGutters key={pd.profileDoctorId}>
            <ListItemButton
              onClick={() => {
                onButtonClick(pd);
                handleListItemClick(pd.profileDoctorId);
              }}
            >
              <ListItemAvatar>
...
const DoctorMessage = () => {
  ...
  const getUserSendMessageToDoctor = async (pd) => {
    ...
    const onConnected = () => {
      stompClient.subscribe(
        "/user/" + pd.profileDoctorId + "/private",
        onPrivateMessage
      );
      // stompClient.subscribe('/user/private', onPrivateMessage);
    };
    ...
    try {
      let res = await authApi().get(
        endpoints["get-user-send-message-to-doctor"](pd.profileDoctorId)
      );
    ...
    const process = async () => {
      try {
        ...
        const res = await authApi().get(
          endpoints["get-message-for-all-view"](selectedProfile, userId)
        );
  ...
  if (current_user === null) <Navigate to="/" />;
      ...
      <div className="Doctor_Message_Wrapper">
        <div className="Doctor_Message">
                    ...
                    {userSendMessageToDoctor.length === 0 ? (
                      ...
                      <>
                        <div className="Profile_List_Info">
                          <ul>
                            {Object.values(userSendMessageToDoctor).map(
                              (usmtd) => {
                                return (
                                  <>
                                    <div
                                      className="Profile_List_Detail"
                                      value={selectedProfile}
                                      onClick={() => {
                                        viewDoctorMessage(usmtd.userId);
                                        setPatientName(
                                          `${usmtd.firstname} ${usmtd.lastname}`
                                        );
                                      }}
                                    >
          ...
          <div className="Doctor_Message_Right">
            <>
              <section>
                <div className="Doctor_Message_Right_Header">
                  <h2 className="text-center mb-4">Tin nhắn</h2>
                </div>
                      ...
                      <div>
                        {userSendMessageToDoctor.length === 0 ? (
                          <>
                            <div className="Doctor_Message_Null">
                              <h5 className="mb-4">
                                Bạn chưa nhắn tin với bệnh nhân nào
                              </h5>
                              <img
                                src={message}
                                alt="Not found"
                          ...
                          <>
                            ...
                            {listMessage.length === 0 ? (
                              <>
                                <div className="Doctor_Message_Null">
                                  <h5 className="mb-4">
                                    Vui lòng chọn bệnh nhân để xem tin nhắn
                                  </h5>
                                  <img
                                    src={message}
                                    alt="Not found"
